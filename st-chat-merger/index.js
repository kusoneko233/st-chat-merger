...
let lastPreviewData = null; // 保存最近一次预览数据，供全屏查看
...
function renderSectionLabel(text, icon, container) {
...
function renderPreviewContent(messages, sectionData, container, truncate) {
    container.innerHTML = '';
    const totalMsgs = messages.length;

    if (sectionData.mode === 'all') {
        renderSectionLabel('全部消息（共 ' + totalMsgs + ' 条）', 'fa-list', container);
    ...
    for (let si = 0; si < merged.length; si++) {
        ...
        renderSectionLabel(sec.labels[0].label, sec.labels[0].icon, container);
        ...
        for (let li = 1; li < sec.labels.length; li++) {
            labelMap.set(sec.labels[li].at, sec.labels[li]);
        }

        for (let i = sec.start; i <= sec.end; i++) {
            if (labelMap.has(i)) {
                const lbl = labelMap.get(i);
                renderSectionLabel(lbl.label, lbl.icon, container);
            }
            renderPreviewMsg(messages[i], i, container, truncate);
        }
...
async function showPreview() {
    ...
    try {
        ...
        // 渲染缩略预览（截断模式）
        renderPreviewContent(messages, sectionData, pe, true);
...
function openFullPreview() {
    ...
    // 渲染完整内容（不截断）
    renderPreviewContent(messages, sectionData, container, false);
